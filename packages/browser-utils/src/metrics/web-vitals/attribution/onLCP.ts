...
const attributeLCP = (metric: LCPMetric): LCPMetricWithAttribution => {
  ...
  if (metric.entries.length) {
    ...
    if (navigationEntry) {
      ...
      const lcpResourceEntry =
        lcpEntry.url &&
        performance.getEntriesByType('resource').filter(e => e.name === lcpEntry.url)[0];
  ...
  const metricWithAttribution: LCPMetricWithAttribution = Object.assign(metric, { attribution });
