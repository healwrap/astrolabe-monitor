...
import { TTFBMetric, TTFBMetricWithAttribution, ReportOpts, TTFBAttribution } from '../types.js';
...
const attributeTTFB = (metric: TTFBMetric): TTFBMetricWithAttribution => {
  ...
  if (metric.entries.length) {
    ...
    const waitEnd = Math.max(
      (navigationEntry.workerStart || navigationEntry.fetchStart) - activationStart,
      0
    );
  ...
  const metricWithAttribution: TTFBMetricWithAttribution = Object.assign(metric, { attribution });
